                    ...
                    <div class="image-item" data-image-id="<?= $img['id'] ?>">
                        <div class="image-container">
                            ...
                            <div class="image-overlay">
                                <button type="button" class="btn-main <?= $img['is_main'] ? 'active' : '' ?>"
                                    onclick="setAsMainImage(<?= $img['id'] ?>, <?= $stamp['id'] ?>)"
                                    title="Définir comme image principale">
                                    <span class="star">★</span>
                                </button>
                                <button type="button" class="btn-delete"
                                    onclick="deleteImage(<?= $img['id'] ?>, <?= $stamp['id'] ?>)"
...
<script>
    ...
    function setAsMainImage(imageId, stampId) {
        if (confirm('Définir cette image comme image principale ?')) {
            // For now, just show an alert - this would require AJAX implementation
            alert('Fonctionnalité à implémenter: définir comme image principale');
        }
    }

    function deleteImage(imageId, stampId) {
        if (confirm('Êtes-vous sûr de vouloir supprimer cette image ? Cette action est irréversible.')) {
            // For now, just show an alert - this would require AJAX implementation
            alert('Fonctionnalité à implémenter: suppression d\'image');
        }
    }
</script>